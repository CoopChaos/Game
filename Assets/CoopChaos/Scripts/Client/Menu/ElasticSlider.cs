...
namespace UnityEngine.UI
{
    ...
    [AddComponentMenu("UI/ElasticSlider", 33)]
    [ExecuteAlways]
    ...
    /// </remarks>
    public class ElasticSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        /// <summary>
        ...
        [SerializeField]
        private float m_ElasticityOffset = 10;
        public float elasticityOffset { get { return m_ElasticityOffset; } set { if (SetPropertyUtility.SetStruct(ref m_ElasticityOffset, value)) { Set(m_Value); UpdateVisuals(); } } }

        [SerializeField]
        private float m_ElasticityFactor = 0;
        public float elasticityFactor { get { return m_ElasticityFactor; } set { if (SetPropertyUtility.SetStruct(ref m_ElasticityFactor, value)) { Set(m_Value); UpdateVisuals(); } } }

        [SerializeField]
        private float m_Elasticity = 1;
        public float elasticity { get { return m_Elasticity; } set { if (SetPropertyUtility.SetStruct(ref m_Elasticity, value)) { Set(m_Value); UpdateVisuals(); } } }
        ...
        public float normalizedValue
        {
            get
            {
                ...
                return (double) minValue != (double) maxValue
                    ? (float) (((double) value - (double) minValue) / ((double) maxValue - (double) minValue))
                    : 0.0f;
            }
            set
            {
        ...
        protected virtual void Update()
        {
            ...
            if (isCurrentlyPressed == false)
            {
                float newValueElastic = Mathf.Clamp(m_Value, minValue - elasticityOffset, maxValue + elasticityOffset);
                float newValue = Mathf.Clamp(m_Value, minValue, maxValue);

                if (newValueElastic != newValue)
                {
                    if (Math.Abs(newValueElastic - newValue) < 0.001f)
                    {
                        newValue = newValueElastic;
                    }
                    else
                    {
                        Set(Mathf.MoveTowards(m_Value, newValue, Time.deltaTime * elasticity), false);
                    }
                }
            }
        }
        ...
        {
            float newValue = Mathf.Clamp(input, minValue - elasticityOffset, maxValue + elasticityOffset);
            float newValueWithoutE = Mathf.Clamp(input, minValue, maxValue);

            if (Mathf.Abs(newValue - newValueWithoutE) < 0.0001f)
            {
                if (wholeNumbers)
                {
            ...
            {
                float diffFactor = Mathf.Abs(input - newValueWithoutE) /
                                   (Mathf.Abs(newValue - newValueWithoutE) * elasticityFactor);

                newValue = Mathf.MoveTowards(newValueWithoutE, newValue,
                    Mathf.Abs(newValue - newValueWithoutE) * diffFactor);
            }
        ...
        void UpdateDrag(PointerEventData eventData, Camera cam)
        {
            ...
            if (clickRect != null && clickRect.rect.size[(int)axis] > 0)
            {
                ...
                float diff = (localCursor - m_Offset)[(int)axis] / clickRect.rect.size[(int)axis];
                
                normalizedValue = (reverseValue ? 1f - diff : diff);
            }
        }
