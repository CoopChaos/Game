...
namespace CoopChaos
{
    ...
    public class CustomNetworkList<T> : NetworkVariableBase where T : unmanaged, IEquatable<T>
    {
        ...
        public CustomNetworkList(NetworkVariableReadPermission readPerm, IEnumerable<T> values) : base(readPerm)
        {
        ...
        /// </summary>
        /// <param name="values">The initial value to use for the NetworkList</param>
        public CustomNetworkList(IEnumerable<T> values)
        {
            foreach (var value in values)
            {
                m_List.Add(value);
